...
Module::Module(Core::System& system) : system(system) {
    ...
    // Get the console ID from the CFG service
    auto cfg = Service::CFG::GetModule(system);
    const u64 console_id = cfg->GetConsoleUniqueId();

    // Concatenate seed and console ID, compute CRC-16/CCITT
    u8 data[16];
    ...
    // Generate the three 4‑digit groups
    const u32 first = (seed & 0xFFFFFFFF) % 10000;
    const u32 second = ((seed >> 32) & 0xFFFFFFFF) % 10000;
    const u32 third = crc % 10000;

    // Pack into u64 (16 bits per group)
    my_friend_key.friend_id = 0;
    my_friend_key.unknown = 0;
    my_friend_key.friend_code = (static_cast<u64>(first) << 32) |
